		...
		foreach ($input['fields'] as $field) {
			...
			if ($field->type == 'relationship') {
				...
				if ($field->relationship_count == 'single') {
					...
					if ($field->title == 'Город вылета') {
						$relationship_column = 'id_cities_from';
					} elseif ($field->title == 'Город прилета') {
						$relationship_column = 'id_cities_to';
					} else {
						$relationship_column = 'id_'.$field->relationship_table_name;
					}

					if ($menu->multilanguage == 1 && $field->lang == 0) {

						$update_multilanguage[$relationship_column] = $field->value;
					}

					$update[$relationship_column] = $field->value;
				...
				if ($field->relationship_count != 'editable') {
					...
					if ($field->relationship_count == 'single') {
						...
						if ($input['id'] == 0) {
							...
							if ($field->title == 'Город вылета') {
								$field_title = 'id_cities_from';
							} elseif ($field->title == 'Город прилета') {
								$field_title = 'id_cities_to';
							} else {
