...
class Pagination extends Component
{
    ...
    public function __construct($count, $pagesize, $page, $paglink, $showmore = false, $pageparam = 'page'){
		...
		$this->fields = [
			'showmore'	=> $s->field('Кнопки', 'Кнопка "Показать больше" (текст)', 'text', true, 'Показать больше'),
		];
	...
	protected function make_pagination ($count, $posts_per_page, $curr, $link) {
		...
		if ( $count != 0){
			...
			if ($curr > $page_count){
				...
				header("Location: ".$link."?".$this->pageparam."=".$page_count);
				die();
			}
	
			if ($curr == 0 || request()->get($this->pageparam) == 1){
				header("HTTP/1.1 301 Moved Permanently");
		...
		if (isset($obj['arrow_left'])){
			$href = $obj['link'];
			$href .= ($obj['arrow_left'] != 1) ? $obj['separator'].'='.$obj['arrow_left'] : '';

			SEO::link_prev($href);
		}

		if (isset($obj['arrow_right'])){
			$href = $obj['link'].$obj['separator'].'='.$obj['arrow_right'];
			SEO::link_next($href);
		}
