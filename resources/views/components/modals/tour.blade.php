@if(!empty($tour))
    <div class="modal-tour fade-in @if($active) active @endif" id="modal-tour">
        <div class="container-modal container-modal-tour">
            ...
            <div class="modal-tour-wrapper">
                ...
                <div class="modal-tour-arrival">
                    ...
                    <x-inputs.button class="center green fit-conten tooltip" tooltip="Для покупки тура вы будете перенаправлены в чат с турагентом!" onclick="open_chat(this)">
                        Купить тур
                    </x-inputs.button>

                </div>

                <div class="modal-tour-agent">

                    <div class="h4 modal-tour-agent-title">Информация о турагенте</div>
                    <x-tour.agent :touragent="$tour->touragent" />
                    ...
                    <div class="modal-tour-agent-option">
                        <div class="modal-tour-agent-option-btns">
                            ...
                            <x-inputs.button href="{{action('DownloadController@downloadPDF', $tour->slug)}}" class="blue-empty">
...
<script>
    ...
    async function open_tour(tour, route){

        tour_slug = tour.getAttribute('data-slug')

        srt = document.location.search
        if (srt.includes('?') ) {
            href = document.location + '&tour=' + tour_slug
        } else {
            href = document.location + '?tour=' + tour_slug
        }
        ...
        if (response.success){
            ...
            $("#tour-modal").html(response.data.html)
    ...
    init_slick();
    //initGmapsTour(document.querySelector('#map').getAttribute('data-title'), document.querySelector('#map').getAttribute('data-star'), document.querySelector('#map').getAttribute('data-lat'), document.querySelector('#map').getAttribute('data-lng'));
