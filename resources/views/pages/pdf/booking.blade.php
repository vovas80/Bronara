...
<html>
  ...
  <body>
    ...
    <table class="table-info">
        ...
        <th>    
            <td style="width: 450px;">
                ...
                <div style="color: #025A9C; font-size: 16px; margin-bottom: 10px;">
                    <b>{{$s->field('Общая информация', 'Цена:', 'text', true, 'Цена:')}}{{round($tour->price)}} {{Region::getCurrency()}}</b>
                </div>

                @if(!empty($tour->status->title))
                    <div style="color: #92A8B8; font-size: 12px;">
                        <b>{{$s->field('Общая информация', 'Статус бронирования:', 'text', true, 'Статус бронирования:')}} {{($tour->status->title)}}</b>
                    </div>
                @endif
            </td>
        </th>
    ...
    @if($tour->peoples->count() > 1)
        <h2>{{$s->field('Информация о туристах', 'Заголовок', 'text', true, 'Информация о туристах')}}</h2>
    @else 
        <h2>{{$s->field('Информация о туристах', 'Заголовок 2', 'text', true, 'Информация о туристe')}}</h2>
    @endif
    <table class="table-touragent">   
        <tr class="table-tourist-wrapper">
            <th>
                <div class="table-tourist-title">{{$s->field('Информация о туристах', 'Номер телефона:', 'text', true, 'Номер телефона:')}}</div>
                <div class="table-tourist-desc">{{$tour->tourist->phone}}</div>
            </th>
            <th>
                <div class="table-tourist-title">{{$s->field('Информация о туристах', 'Почта:', 'text', true, 'Почта:')}}</div>
                <div class="table-tourist-desc">{{$tour->tourist->email}}</div>
            </th>
        </tr>
        @foreach ($tour->peoples as $tourist)
            
            <tr class="table-tourist-wrapper">
                <th>
                    <div class="table-tourist-title">{{$s->field('Информация о туристах', 'ФИО:', 'text', true, 'ФИО:')}}</div>
                    <div class="table-tourist-desc">{{$tourist->second_name}} {{$tourist->first_name}} {{$tourist->third_name}}</div>
                </th>
                <th>
                    <div class="table-tourist-title">{{$s->field('Информация о туристах', 'Дата рождения:', 'text', true, 'Дата рождения:')}}</div>
                    <div class="table-tourist-desc">{{$tourist->date_of_birth}}</div>
                </th>
                <th>
                    <div class="table-tourist-title">{{$s->field('Информация о туристах', 'Серия и номер паспорта:', 'text', true, 'Серия и номер паспорта:')}}</div>
                    <div class="table-tourist-desc">{{$tourist->passport}}</div>
                </th>
                <th>
                    <div class="table-tourist-title">{{$s->field('Информация о туристах', 'Действует до:', 'text', true, 'Действует до:')}}</div>
                    <div class="table-tourist-desc">{{$tourist->valid}}</div>
                    
                </th>
                <th>
                    <div class="table-tourist-title">{{$s->field('Информация о туристах', 'Кем выдан паспорт:', 'text', true, 'Кем выдан паспорт:')}}</div>
                    <div class="table-tourist-desc">{{$tourist->authority}}</div>
                </th>
                <th>
                    <div class="table-tourist-title">{{$s->field('Информация о туристах', 'Гражданство:', 'text', true, 'Гражданство:')}}</div>
                    <div class="table-tourist-desc">{{$tourist->citizenship->title}}</div>
                </th>
            </tr>
